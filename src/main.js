...
export default class StepZilla extends Component {
  ...
  jumpToStep(evt) {
    if (evt.currentTarget == undefined) {
      ...
      if (!this.props.stepsNavigation || evt.currentTarget.value === this.state.compState) { // if stepsNavigation is turned off or user clicked on existing step again (on step 2 and clicked on 2 again) then ignore
        evt.preventDefault();
        evt.stopPropagation();
      ...
      const movingBack = evt.currentTarget.value < this.state.compState; // are we trying to move back or front?
          ...
          if (proceed) {
            if (!movingBack) {
              ...
              passThroughStepsNotValid = this.props.steps
                .reduce((a, c, i) => {
                  if (i >= this.state.compState && i < evt.currentTarget.value) {
                    a.push(c.validated);
                  }
                  return a;
                }, [])
    ...
    return (
      <div className="multi-step" onKeyDown={(evt) => {this.handleKeyDown(evt)}}>
...

          {
              ...
              ? <ol className={"progtrckr" + (this.props.progressTrackerCls ? " " + this.props.progressTrackerCls : "")}>
                      {this.renderSteps()}
                  </ol>
              : <span></span>
          }
